		...
		brk, BOOLEAN cnt)
{
	...
	switch(node->name) {
		case N_STMT_LABEL_CASE: 
			...
			lit=getTypeAndValueOfExpression(node->llink); // 수식 value, type
	...
	switch (id->kind) {
		case ID_VAR:
			...
			if (isArrayType(id->type) && id->type->expr == 0) 
				semantic_error(86, id->line); // 배열크기 점검

			if (i % 4) i = i / 4 * 4 + 4; // 정렬

			if (id->specifier == S_STATIC) id->level = 0; // 지역변수로 전환

			if (id->level == 0) { 
				id->address = global_address; // 전역
				global_address += i; 
			} else {  // 전역 변수 사이즈
				id->address = addr; // 지역
				result = i; 
