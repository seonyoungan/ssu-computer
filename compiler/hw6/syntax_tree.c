...
A_SPECIFIER *updateSpecifier(A_SPECIFIER *p, A_TYPE *t,S_KIND s)
{
    if(t)
        if(p->type)
            if (p->type==t)
                ;
            else    syntax_error(24,0); //교재는 syntax_error(24);
        else
            p->type=t;
    ...
    if(s)
    {
        if(p->stor)
            if(s==p->stor)
                ...
                syntax_error(24,0); //교재는 syntax_error(24);
        else p->stor=s;
    }
    return (p);
}
...
A_ID *setFunctionDeclaratorSpecifier(A_ID *id,A_SPECIFIER *p)
{
    ...
    //check storage class
    if(p->stor)
        syntax_error(25,0);  //교재는 syntax_error(25)
    ...
    if (a)
        ...
        {
        //check prototype: parameter and return type
        ...
        while (a)
        {
            if(strlen(a->name))
                ...
                syntax_error(23, 0); //교재는 syntax_error(23);
            a=a->link;
        }
        return (id);
}
...
A_ID *setParameterDeclaratorSpecifier(A_ID *id,A_SPECIFIER *p)
{
    ...
    if (p->stor || p->type==void_type)
        syntax_error(14,0); //교재는 syntax_error(14);
    setDefaultSpecifier(p);
    id=setDeclaratorElementType(id,p->type);
    id->kind=ID_PARM;
    return (id);
...
A_TYPE *setTypeStructOrEnumIdentifier(T_KIND k,char *s,ID_KIND kk)
{
    ...
    //make a new struc or enum identifier
    id=makeIdentifier(s);
    t=makeType(k);
    id->type=t;
    id->kind=kk;
...
BOOLEAN isPointerOrArrayType(A_TYPE *t) //교재에 없는 내용 추가
{
...
void syntex_error(int i, char *s)
{
    syntax_err++;
    ...
    switch(i)
    {
        ...
        case 13: printf("undefined identifier  %s", s);
            break;
        case 14: printf("illegal type specifier in formal parameter %s", s);
            break;
        case 20: printf("illegal storage class in type specifiers %s", s);
            break;
        case 21: printf("illegal function declarator %s", s);
            break;
        case 22: printf("conflicting parm type in prototype function %s", s);
            break;
        case 23: printf("empty parameter name %s", s);
            break;
        case 24: printf("illegal declaration specifiers %s", s);
            break;
        case 25: printf("illegal function specifiers %s", s);
            break;
        case 26: printf("illegal or conflicting return type infunction %s", s);
            break;
